...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) < smallest and len(grid.get_cells()[i][j]) > 1:
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q: # while Q is not empty
            var = Q.pop() # var = Q.pop()
            vars_assigned_rows, row_failure = self.remove_domain_row(grid, var[0], var[1])
            vars_assigned_columns, column_failure = self.remove_domain_column(grid, var[0], var[1])
            vars_assigned_units, unit_failure = self.remove_domain_unit(grid, var[0], var[1])


            if True in [row_failure,column_failure,unit_failure]:
                return True
            

            for v in vars_assigned_rows: Q.add(v)
            for v in vars_assigned_columns: Q.add(v)
            for v in vars_assigned_units: Q.add(v)
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        """
        # note: I am running preprocessing step in the main so that I dont have to initialize ac3

        if grid.is_solved(): return grid # if A is complete: return A
        coords = var_selector.select_variable(grid) # var = select_unassigned_var(A)
        for d in grid.get_cells()[coords[0]][coords[1]]: # for d in domain(var)
            if grid.is_value_consistent(d, coords[0], coords[1]): # if d is consistent with A
                copy_g = grid.copy() # copy_A = A.copy()
                copy_g.get_cells()[coords[0]][coords[1]] = d # {var = d} in copy_A
                r = ac3.consistency(copy_g, {coords}) # Changed to adapt to consistency
                if r == False:
                    rb = self.search(copy_g,var_selector) # rb = Backtracking(copy_A)
                    if rb.is_solved(): # if rb is not failure
                        return rb
        return grid # return failed grid
...
for p in problems:
    ...
    #Print the initial sukodu/unsolved grid
    #g.print()

    # Instance of AC3 Object
    ac3 = AC3()

    #Timer for preprocessing Step
    preproc_start = time.perf_counter()
    ac3.pre_process_consistency(g)
    preproc_end = time.perf_counter() - preproc_start

    #Timer
    start = time.perf_counter()
    chosen_var_selector = MRV()
    solved_grid = searcher.search(g, chosen_var_selector)
    running_time_mrv.append(time.perf_counter()-start + preproc_end) # Add preprocess time to MRV
    assert solved_grid.is_solved(), "Failed to Solve Sudoku"

    start = time.perf_counter()
    chosen_var_selector = FirstAvailable()
    solved_grid = searcher.search(g, chosen_var_selector)
    running_time_first_available.append(time.perf_counter()-start + preproc_end) # Add preprocess time to FA
    assert solved_grid.is_solved(), "Failed to Solve Sudoku"

    #Print the solved grid/sudoku
    #solved_grid.print()
